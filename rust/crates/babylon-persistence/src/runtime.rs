...
pub(crate) fn require_active_authority(
    config: &Config,
) -> Result<crate::CurrentSchemaIdentity, RustPersistenceRuntimeErrorV2> {
    validate_connection_target(config)
        .map_err(|_| RustPersistenceRuntimeErrorV2::ActivationRequired)?;
    let mut client = crate::current_schema::bounded_config(config)
        .connect(NoTls)
        .map_err(|error| {
            RustPersistenceRuntimeErrorV2::postgres("connect current schema reader", &error)
        })?;
    require_active_authority_client(&mut client)
}
...
pub(crate) fn require_active_authority_client(
    ...
    crate::current_schema::require_current_schema(client)
        .map_err(RustPersistenceRuntimeErrorV2::CurrentSchema)
}
