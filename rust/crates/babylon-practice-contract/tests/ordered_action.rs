...
use babylon_practice_contract::{
    ...
    PracticeIntent, PracticeIntentError, PracticeParameter, PracticeTargetIdentity,
    PracticeTargetTag, ProposalNonce, ResolvedPracticeBatch, ResolvedPracticeBatchError,
    ResolvedPracticeBatchItem, TaggedPracticeTarget, MAX_RESOLVED_PRACTICE_BATCH_ITEMS,
};
...
fn action_id_preimage_is_exact_and_excludes_the_ordinal() {
    ...
    assert_eq!(
        encode_practice_action_id_preimage(&replay, &value).unwrap(),
        expected
    );
    assert_eq!(
        practice_action_id(&replay, &value).unwrap().as_bytes(),
        &hex_digest("955c45b85e469ccdfcbee405d6899fc00d0c6bb882daa32e6f2bfa072fe181a6")
    );
    assert_eq!(
        encode_practice_action_id_preimage(&session("!"), &value)
            .unwrap()
            .len(),
        69
    );
    ...
    assert_eq!(
        encode_practice_action_id_preimage(&session(&maximum_session), &value)
            .unwrap()
            .len(),
        324
    );

    let original_only = OrderedPracticeActionBatch::project(
        session("replay-A"),
        &batch(vec![item(0x70)]),
        &ledger(),
    )
    .unwrap();
    let lower_inserted = OrderedPracticeActionBatch::project(
        session("replay-A"),
...
fn exact_empty_batch_binds_session_and_tick() {
    let value = OrderedPracticeActionBatch::empty(session("replay-A"), 11).unwrap();
    let expected = hex_bytes(concat!(
    ...
    let other_session = OrderedPracticeActionBatch::empty(session("replay-B"), 11).unwrap();
    let other_tick = OrderedPracticeActionBatch::empty(session("replay-A"), 12).unwrap();
...
fn projector_assigns_contiguous_ordinals_and_exact_canonical_bytes() {
    ...
    let value =
        OrderedPracticeActionBatch::project(session("replay-A"), &source, &ledger()).unwrap();
    let expected = hex_bytes(concat!(
        "626162796c6f6e2e6f7264657265642d70726163746963652d616374696f6e2d62617463682e7631",
        "00",
        "0001",
...
fn projector_refuses_every_untrusted_source_shape_before_projection() {
    ...
    assert_eq!(
        OrderedPracticeActionBatch::project(session("replay-A"), &valid, &wrong_ledger),
        Err(OrderedPracticeActionError::Source(
            ResolvedPracticeBatchError::Batch(PracticeBatchError::BatchLedgerDigest)
        ))
    );

    let unordered = batch(vec![item(0x70), item(0x60)]);
    assert_eq!(
        OrderedPracticeActionBatch::project(session("replay-A"), &unordered, &trusted),
        Err(OrderedPracticeActionError::Source(
            ResolvedPracticeBatchError::Batch(PracticeBatchError::BatchItemOrder)
        ))
    );
    let duplicate = batch(vec![item(0x60), item(0x60)]);
    assert_eq!(
        OrderedPracticeActionBatch::project(session("replay-A"), &duplicate, &trusted),
        Err(OrderedPracticeActionError::Source(
            ResolvedPracticeBatchError::Batch(PracticeBatchError::BatchItemDuplicate)
        ))
    );
    ...
    assert_eq!(
        OrderedPracticeActionBatch::project(session("replay-A"), &wrong_tick, &trusted),
        Err(OrderedPracticeActionError::Source(
            ResolvedPracticeBatchError::Batch(PracticeBatchError::BatchResolveTick)
        ))
    );
    ...
    assert_eq!(
        OrderedPracticeActionBatch::project(session("replay-A"), &oversized_intent, &trusted),
        Err(OrderedPracticeActionError::Source(
            ResolvedPracticeBatchError::Intent(PracticeIntentError::IntentParameterLength)
        ))
    );

    let too_many = batch(vec![item(0x60); MAX_RESOLVED_PRACTICE_BATCH_ITEMS + 1]);
    assert_eq!(
        OrderedPracticeActionBatch::project(session("replay-A"), &too_many, &trusted),
        Err(OrderedPracticeActionError::Source(
            ResolvedPracticeBatchError::Batch(PracticeBatchError::BatchItemLimit)
        ))
    );
}
