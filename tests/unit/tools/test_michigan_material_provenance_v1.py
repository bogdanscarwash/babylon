...
def test_design_does_not_turn_observed_jobs_or_wages_into_material_quantities() -> None:
    ...
    topology = json.loads((ROOT / contract["topology"]["path"]).read_text())
    assert len(topology["processes"]) == 5
    ...
    parameters = tomllib.loads((ROOT / contract["parameters"]["path"]).read_text())
    assert all(
        "WAGE" not in key and "QCEW" not in key
        for process in parameters["process"].values()
        for key in process
    )
